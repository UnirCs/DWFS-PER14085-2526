...
function redConverter() {
  ...
  //Aqui tu codigo
   const redPixels = pixels.map(row =>
    {row.map(pixel => [pixel[0], 0, 0])});
...
function greenConverter() {
  ...
  let pixels = handler.getPixels();
  //Aqui tu codigo
  const grenPixels = pixeles.map(row =>
    {row.map(pixel => [0, pixel[1], 0])});
...
function blueConverter() {
  ...
  let pixels = handler.getPixels();
  //Aqui tu codigo
  const bluePixels = pixels.map(row =>
    {row.map(pixel => [0,0, pixel[2]])});
...
function greyConverter() {
  ...
  const greyPixels = pixels.map(row =>{
    row.map(pixel => {
      const median =([0] + pixel[1] + pixel[2] / 3);
      return [md, md, md]
    }
  )    
});
...
function blackAndWhiteConverter() {
  ...
  let pixels = handler.getPixels();
  //Aqui tu codigo
  const negroyblancoPixels = pixels.map(row =>
    {row.map(pixel => {
      const md = ([0],pixel[1]+ pixel[2] / 3);
      return md < 128 ? [0,0,0] : [255,255,255]
    }
  )
});
  handler.savePixels(pixels, outputPath);
...
function scaleDown() {
  ...
  //Aqui tu codigo
   ...
   for(let row = 0; row < newHeight; row+=2) {
       for(let col = 0; col < newWidth; col+=2) {
        const origianlIdx = (row * width + col) * channels;
              
           for(let c = 0; c < channels; c++) {
               scaleDownPixels.push(pixels[origianlIdx + c]);
           }
          }
        }
...
function dimBrightness(dimFactor) {
  ...
  let pixels = handler.getPixels();
  //Aqui tu codigo
  const dimBrightnessPixels = pixels.map(row =>{
    row.map(pixel => [pixel[0] / dimFactor, pixel[1] / dimFactor, pixel[2] / dimFactor]) });
...
function invertColors() {
  ...
  let pixels = handler.getPixels();
  //Aqui tu codigo
   const invertColorsPixels = pixels.map(row =>{
    row.map(pixel => [255 - pixel[0], 255 - pixel[1], 255 - pixel[2]])});
...
function merge(alphaFirst, alphaSecond) {
  ...
  let dogPixels = dogHandler.getPixels();
      //Aqui tu codigo
   const pixels= catPixels.map((row, rowIndex) =>
