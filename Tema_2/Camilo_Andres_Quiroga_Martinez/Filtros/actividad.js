...
function greyConverter() {
  ...
  for (let i = 0; i < filas; i++) {
      for (let j = 0; j < columnas; j++) {
        let gris = (pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2]) / 3;
        pixels[i][j][0] = gris;
        pixels[i][j][1] = gris;
        pixels[i][j][2] = gris;
...
function scaleDown() {
  ...
  let filasOriginal = pixels.length;
  let columnasOriginal = pixels[0].length;

  let nuevasFilas = Math.floor(filasOriginal / 2);
  let nuevasColumnas = Math.floor(columnasOriginal / 2);

  let newPixels = [];  // Nuevo array para la imagen escalada

  for (let i = 0; i < nuevasFilas; i++) {
      let fila = [];
      for (let j = 0; j < nuevasColumnas; j++) {
          fila.push(pixels[i * 2][j * 2]);
      }
      newPixels.push(fila);
  }
...
function dimBrightness(dimFactor) {
  ...
  for (let i = 0; i < filas; i++) {
      for (let j = 0; j < columnas; j++) {
        pixels[i][j][0] = pixels[i][j][0] / 6; // El divisor es el número que se le quiere reducir de brillo a cada canal (mayor número, menor brillo)
        pixels[i][j][1] = pixels[i][j][1] / 6;
        pixels[i][j][2] = pixels[i][j][3] / 6;
